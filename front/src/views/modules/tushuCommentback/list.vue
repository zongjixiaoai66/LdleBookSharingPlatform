        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('tushuCommentback','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('tushuCommentback','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('tushuCommentback','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('tushuCommentback','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tushufenxiangpingtai/upload/tushuCommentbackMuBan.xls"
                        >批量导入图书评价数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('tushuCommentback','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="tushuCommentbackUploadSuccess"
                                :on-error="tushuCommentbackUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('tushuCommentback','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入图书评价数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('tushuCommentback','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "tushuCommentback.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('tushuCommentback','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="tushuCommentbackText"
                                      header-align="center"
                                      label="评价内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.tushuCommentbackText != null &&scope.row.tushuCommentbackText.length>10">
                                {{scope.row.tushuCommentbackText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.tushuCommentbackText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('tushuCommentback','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('tushuCommentback','修改') && sessionTable=='yonghu' && userId==scope.row.tushuYonghuId" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('tushuCommentback','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'tushu_commentback',//当前表表名,
                        sumColum : 'tushu_commentback_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'tushu_commentback_name',//分组字符串字段
                        // types : 'tushu_commentback_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "tushu_commentback_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['tushuCommentbackDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "tushuCommentback/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            tushuCommentbackUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "tushuCommentback/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            tushuCommentbackUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "tushuCommentback/update",
                        method: "post",
                        data: {
                            id:id,
//                            tushuCommentbackTypes:tushuCommentbackTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
